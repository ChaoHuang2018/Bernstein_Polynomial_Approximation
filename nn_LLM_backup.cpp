...
int main()
{
	...
	Expression_AST<Real> deriv_x0("1.4*x2-0.9*x0");  // theta_r = 0
	Expression_AST<Real> deriv_x1("2.5*x4-1.5*x1");  // theta_r = 0
	Expression_AST<Real> deriv_x2("u");  // theta_r = 0
	Expression_AST<Real> deriv_x3("2-1.3*x2*x3");  // theta_r = 0
	Expression_AST<Real> deriv_x4("0.7*x0-x3*x4");
	Expression_AST<Real> deriv_x5("0.3*x0-3.1*x5");
	Expression_AST<Real> deriv_x6("1.8*x5-1.5*x1*x6");
...
setting.printOff();
	 ...
	 */
	Interval init_x0(1.1,1.3), init_x1(0.95,1.15), init_x2(1.4, 1.6), init_x3(2.3, 2.5), init_x4(0.9, 1.1), init_x5(0, 0.2), init_x6(0.35, 0.55), init_u(0);
...
*/		
		...
		string strBox = "[" + box[0].toString() + "," + box[1].toString() + "," + box[2].toString() + "," + box[3].toString() + box[4].toString() + box[5].toString() + box[6].toString() + "]";
//cout << strBox <<endl;
...
*/
    ...
    time(&end_timer);
    seconds_time = difftime(start_timer, end_timer);
	...
	ofstream result_output("./outputs/nn_LLM.txt");
	if (result_output.is_open())
	{
		result_output << err_max << endl;
		result_output << seconds_time << endl;
	}

	// you need to create a subdir named outputs
	// the file name is example.m and it is put in the subdir outputs
	plot_setting.plot_2D_interval_MATLAB("nn_LLM", result);
